...
def linear_regression_visualization(lr, iters, fps, if_save, file_path):
    ...
    y = x**2 + (np.random.rand(500, 1) * 2 - 1)
    ...
    # 生成平滑的 x 点用于绘制曲线
    x_curve = np.linspace(-1, 1, 100).reshape(-1, 1)  # 创建等间距点
    ...
    ax.plot(x_curve.data, x_curve.data**2, color="lightgreen", label="Target Function")
    ...
    ax.legend()
    ax.grid(True)
    ax.set_title("y = x**2 Neural Network Training")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
...
if __name__ == "__main__":
    file_path = os.path.join(".", "Grad", "linear_regression_x^2_small.gif")
    linear_regression_visualization(lr=0.4, iters=10000, fps=30, if_save=False, file_path=file_path)
