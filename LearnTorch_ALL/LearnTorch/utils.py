...
def _dot_func_(f, direction): # 函数Function转化为dot语言的函数
    ...
    if direction == "Forward": # 如果要生成正向传播的图
        ...
        for x in f.inputs: # 函数节点指向所有输入节点
            txt += dot_edge.format(id(f), id(x))
        for y in f.outputs:# 全部输出节点指向函数节点
            txt += dot_edge.format(id(y()), id(f)) #因为y是weakref，所以要用y()来引用
    return txt
...
def plot_dot_graph(output, save_file, verbose=False, to_file='graph.png', file_path='CGMap'):
    ...
    # 输出：dot语言代码
    dot_graph_forward = get_dot_graph(output=output , direction = "Forward", verbose=verbose)
    dot_graph_backward = get_dot_graph(output=output , direction = "Backward", verbose=verbose)
    ...
    if save_file:
        ...
        try:
            ...
            return display.Image(to_file_forward_path)
        except:
            pass
