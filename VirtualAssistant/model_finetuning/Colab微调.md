...
​	1.3 首先下载最新的`Unsloth`库，点击左侧的**运行按钮**，以下代码全部按照点击左侧运行按钮执行
...
dataset = load_dataset(
    "json", 
    data_files="/content/drive/MyDrive/dataset/huanhuan.json", 
    split="train"  # 加载训练集
)
...
```
model.save_pretrained("/content/drive/MyDrive/lora_model") # 云盘保存
...
if True:
    from unsloth import FastLanguageModel
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="/content/drive/MyDrive/lora_model",  # 微调模型的路径
        max_seq_length=max_seq_length,  # 最大序列长度
        dtype=dtype,  # 数据类型
        load_in_4bit=load_in_4bit,  # 是否启用 4bit 量化
    )
