...
# 按钮跳转到教程部分
if st.button("😀 我是新手!!! 📖 查看学习率与迭代次数的详细教程"):
    st.write(
        r"""
        ---
        ---
        ...
        这两个概念在训练模型时非常关键，直接决定了我们模型的训练效果和速度。🚀
        ...
        - **迭代次数太少**：训练不足，模型可能没有学到足够的知识，效果不好⚠️。
        ...
        🔑 **小技巧**：
        - 开始时可以选择一个较小的学习率（例如 0.001），逐渐增加📈。
        - 选择适当的迭代次数，避免过度训练（过拟合）❗。
        - 实践中，你可以通过 **网格搜索（Grid Search）** 或 **随机搜索（Random Search）** 来找到最佳的超参数（学习率和迭代次数）🔍。

        希望这篇讲解能够帮助你更好地理解学习率和迭代次数的作用！🤓
